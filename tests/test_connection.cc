...
int main()
{
	...
	auto test_address_resolving = [&](connection & conn)
	{
	...
	{
		connection udpConn("cxemotexnika.org",5683);
		LOG(DEBUG, "Test 2 for IP address resolving :");
		test_address_resolving(udpConn);
	}
	{
		connection udpConn("google.com.ua",5683);
		LOG(DEBUG, "Test 3 for IP address resolving :");
		test_address_resolving(udpConn);
	}
	{
		connection udpConn("192.168.0.100",5683);
		LOG(DEBUG, "Test 4 for IP address resolving :");
		test_address_resolving(udpConn);
	}
	{
		connection udpConn("2001:67c:2e5c:2033:30b9:b34d:ff24:7850",5683);
